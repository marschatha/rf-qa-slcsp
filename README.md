...
Make sure you have Ruby 3.1.2 installed on the system.
You can run the code via command line using the raketask `rake generate_slcsp_csv`.

## Running Tests

To run tests you will have to install Rspec using `bundle` and run the test via command line `rspec`.
There is only one high level test at the moment due to the 3 hour time limitation in the instructions, but it does cover various edge cases and scenarios.
